...
pub const JNINativeInterface = extern struct {
    ...
    define_class: ?fn (env: [*c]JNIEnv, name: [*c]const u8, loader: jobject, buf: [*c]const jbyte, len: jsize) callconv(.C) jclass,
    ...
    get_superclass: ?fn (env: [*c]JNIEnv, jclass) callconv(.C) jclass,
    is_assignable_from: ?fn (env: [*c]JNIEnv, jclass, jclass) callconv(.C) jboolean,
    to_reflected_field: ?fn (env: [*c]JNIEnv, jclass, jfield_id, jboolean) callconv(.C) jobject,
    ...
    get_direct_buffer_capacity: ?fn (env: [*c]JNIEnv, jobject) callconv(.C) jlong,
    get_object_ref_type: ?fn (env: [*c]JNIEnv, jobject) callconv(.C) JobjectRefType,
    get_module: ?fn (env: [*c]JNIEnv, jclass) callconv(.C) jobject,
};
...
pub const JNIInvokeInterface = extern struct {
    ...
    reserved1: ?*anyopaque,
    reserved2: ?*anyopaque,
    destroy_java_vm: ?fn ([*c]JavaVM) callconv(.C) jint,
    attach_current_thread: ?fn ([*c]JavaVM, [*c]?*anyopaque, ?*anyopaque) callconv(.C) jint,
    detach_current_thread: ?fn ([*c]JavaVM) callconv(.C) jint,
    get_env: ?fn ([*c]JavaVM, [*c]?*anyopaque, jint) callconv(.C) jint,
    attach_current_thread_as_daemon: ?fn ([*c]JavaVM, [*c]?*anyopaque, ?*anyopaque) callconv(.C) jint,
};
