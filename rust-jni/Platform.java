...
public class Platform {
  ...
  /**
   * Compares the platform version with the supplied version. The version must be of the form
   * a[.b[.c[.d...]]] where a, b, c, d, ... are decimal integers.
   *
   * @throws NullPointerException if the parameter is null
   * @throws NumberFormatException if there is an error parsing either version as split into
   *     component strings
   * @return -1, 0, or 1 according to whether the platform version is less than, equal to, or
   *     greater than the supplied version
   */
  public static int compareOsVersion(String version) {
    ...
    List<Integer> s1 =
        Arrays.stream(osVersionTokens).map(Integer::valueOf).collect(Collectors.toList());
    List<Integer> s2 =
        Arrays.stream(version.split("\\.")).map(Integer::valueOf).collect(Collectors.toList());
  ...
  /**
   * Return a boolean for whether we expect to be able to attach the SA to our own processes on this
   * system. This requires that SA is ported/available on this platform.
   */
  ...
  /**
   * On Linux, first check the SELinux boolean "deny_ptrace" and return false as we expect to be
   * denied if that is "1". Then expect permission to attach if we are root, so return true. Then
   * return false for an expected denial if "ptrace_scope" is 1, and true otherwise.
   */
  ...
  private static boolean isArch(String archnameRE) {
    return Pattern.compile(archnameRE, Pattern.CASE_INSENSITIVE).matcher(osArch).matches();
  }
  ...
  public static void addLibraryPath(String path) {
    try {
      ...
      Field field = classLoaderClass.getDeclaredField("usr_paths");
      field.setAccessible(true);
      String[] paths = (String[]) field.get(classLoaderClass);
      ...
      field.set(classLoaderClass, tmp);
    } catch (IllegalAccessException | NoSuchFieldException e) {
      throw new RuntimeException("Failed to add library path");
    }
  }
}
